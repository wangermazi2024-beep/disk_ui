...
pub fn spawn_scan(root: PathBuf, tx: Sender<ScanMessage>) {
    std::thread::spawn(move || {
        ...
        match run_scan(&root, &counter, &cancel, &tx) {
            Ok(mut node) => {
                if let Some(info) = &disk_info {
                    #[cfg(windows)]
...
fn run_scan(
    ...
    let hardlink_seen: Arc<dashmap::DashSet<u64>> = Arc::new(dashmap::DashSet::new());

    rayon::scope(|s| {
        process_dir(
            s, root.to_path_buf(), 0, root_task, root_slot.clone(),
            counter.clone(), cancel.clone(), tx.clone(), hardlink_seen,
        );
    });
...
fn process_dir<'scope>(
    ...
    cancel: Arc<AtomicBool>,
    tx: Sender<ScanMessage>,
    hardlink_seen: Arc<dashmap::DashSet<u64>>,
) {
    ...
    for e in entries {
        if e.is_dir {
            ...
            // 修正压缩文件的物理大小：GetFileInformationByHandleEx 批量枚举里返回的
            // AllocationSize，对 NTFS 压缩文件不准（它反映的是"如果不压缩需要分配多少"，
            // 不是压缩后实际占用了多少簇），只有 GetCompressedFileSizeW 才能拿到真实压缩后大小。
            // 这也是为什么之前物理大小算出来几乎等于逻辑大小，看起来像"物理/逻辑搞反了"——
            // 其实字段没搞反，是压缩文件的物理大小没算对。稀疏文件的 AllocationSize 本身
            // 就是准的，不用额外处理。压缩文件占比很小，这里只对它们单独调一次 API，
            // 不会把"逐文件开 handle"的老问题带回来。
            #[cfg(windows)]
            const FILE_ATTRIBUTE_COMPRESSED: u32 = 0x800;
            #[cfg(windows)]
            let physical = if e.attrs & FILE_ATTRIBUTE_COMPRESSED != 0 {
                get_compressed_size(&path.join(&e.name)).unwrap_or(e.physical)
            } else {
                e.physical
            };
            #[cfg(not(windows))]
            let physical = e.physical;

            // 硬链接去重：批量枚举已经免费拿到了 FileId（NTFS 文件参考号，卷内唯一），
            // 不需要再像老版本那样为了拿链接信息去逐文件开 handle。同一个 FileId 在整个扫描里
            // 只在第一次出现时计入大小，后面重复出现的（真正的硬链接）大小记 0，
            // 但节点还是会保留在树里方便浏览。这解决了"逻辑大小比实际磁盘占用多出一截"
            // 里由硬链接造成的那部分。
            let (logical, physical) = if e.file_id != 0 && !hardlink_seen.insert(e.file_id) {
                (0, 0)
            } else {
                (e.logical, physical)
            };
            ...
            leaf_nodes.push(Node::new_file_with_meta(
                e.name, logical, physical, file_color(),
...
/// 用 GetCompressedFileSizeW 拿压缩文件真实的磁盘占用（批量枚举返回的 AllocationSize
/// 对压缩文件不准，只有这个 API 是准的）。只对带 FILE_ATTRIBUTE_COMPRESSED 的文件调用，
/// 数量少，不构成性能问题。
#[cfg(windows)]

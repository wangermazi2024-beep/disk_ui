...
// 非 Windows 平台给一个空的 main，让这个 bin 也能编过。
#[cfg(not(windows))]
fn main() {
...
fn main() {
    ...
    eprintln!("[verify] 总容量: {:.2} GB ({})", total as f64 / 1e9, total);
    eprintln!("[verify] 已用:   {:.2} GB ({})", used as f64 / 1e9, used);
    eprintln!("[verify] 可用:   {:.2} GB ({})", free as f64 / 1e9, free);
    eprintln!();
    ...
    eprintln!("[verify] MFT 文件数:   {}", mft_root.file_count);
    eprintln!("[verify] MFT 文件夹数: {}", mft_root.folder_count);
    eprintln!("[verify] MFT 大小汇总: {:.2} GB ({})", mft_root.size as f64 / 1e9, mft_root.size);
    eprintln!();
    ...
    eprintln!("[verify] 标准遍历 大小汇总: {:.2} GB ({})", std_root.size as f64 / 1e9, std_root.size);
    eprintln!();
    ...
    eprintln!("├──────────────┼────────────────┼────────────────┼────────────────┤");
    eprintln!("│ 文件数       │ {:>14} │ {:>14} │ {:>14} │", "—", mft_root.file_count, std_root.file_count);
    eprintln!("│ 文件夹数     │ {:>14} │ {:>14} │ {:>14} │", "—", mft_root.folder_count, std_root.folder_count);
    eprintln!("│ 大小 (字节)  │ {:>14} │ {:>14} │ {:>14} │", used, mft_root.size, std_root.size);
    eprintln!("│ 大小 (GB)    │ {:>14.2} │ {:>14.2} │ {:>14.2} │",
        used as f64 / 1e9, mft_root.size as f64 / 1e9, std_root.size as f64 / 1e9);
    eprintln!("└──────────────┴────────────────┴────────────────┴────────────────┘");
    eprintln!();
    ...
    eprintln!("  文件数差 (MFT - 标准遍历)   = {} {}",
        file_diff,
        if file_diff > 0 { "(MFT 多看到一些 — 正常，标准遍历被权限挡住)" }
        else if file_diff < 0 { "⚠ 标准遍历多看到一些 — 不正常！" }
        else { "(完全一致)" });
    eprintln!("  文件夹数差 (MFT - 标准遍历) = {} {}",
        folder_diff,
        if folder_diff >= 0 { "(MFT >= 标准遍历，正常)" }
        else { "⚠ 标准遍历多看到一些 — 不正常！" });
    eprintln!("  大小差 (MFT vs 标准遍历)    = {:+.2}%", size_diff_pct);
    eprintln!("  大小差 (系统已用 vs MFT)    = {:+.2}%  (预期 5%~25%，含簇碎片/元数据/卷影)",
...
fn run_standard_only(root_path: &str, drive_letter: char, total: u64, used: u64) {
    ...
    eprintln!("  系统已用: {:.2} GB, 标准遍历汇总: {:.2} GB, 差: {:+.2}%",
        used as f64 / 1e9, std_root.size as f64 / 1e9,

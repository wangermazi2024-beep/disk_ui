...
fn walk(
    ...
    // 用显式栈代替原生递归：目录树够深（比如几千层的备份/嵌套 node_modules）时，
    // 原生递归会栈溢出崩溃。栈里存的是"还没写行的节点"，出栈时才写这一行，
    // 子节点按倒序 push，这样出栈顺序（先序、从左到右）和原递归版完全一致。
    let mut stack: Vec<(&Node, String, u64)> = Vec::new();
    for child in node.children.iter().rev() {
        stack.push((child, join_path(cur_path, &child.name), node.logical_size.max(1)));
    }
    while let Some((cur, path, parent_size)) = stack.pop() {
        let parent_pct = if parent_size > 0 { cur.logical_size as f64 / parent_size as f64 } else { 0.0 };
        let total_pct = if disk_logical > 0 { cur.logical_size as f64 / disk_logical as f64 } else { 0.0 };
        write_row(f, &path, cur, parent_pct, total_pct, parent_size, disk_logical)?;
        if cur.is_folder() {
            *folder_count += 1;
            let cur_parent_size = cur.logical_size.max(1);
            for child in cur.children.iter().rev() {
                stack.push((child, join_path(&path, &child.name), cur_parent_size));
            }
        } else {
            *file_count += 1;
        }

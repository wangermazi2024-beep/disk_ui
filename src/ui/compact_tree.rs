...
//!
//! 点击/右键判定用的是 egui_extras::TableRow::response()——官方 table_demo.rs 里
//! 演示的标准做法（整行所有单元格 Response 的并集），而不是自己在每个格子里手动
//! allocate_rect+Sense::click() 再一个个判断：手动那套很容易漏掉格子之间的空隙
//! （之前就出过这个 bug），用官方提供的整行 Response 从根上不会有这个问题，
//! 代码也短很多，不用自己再造一遍轮子。
...
pub fn show(ui: &mut egui::Ui, root: &Node, selected: &Option<NodePath>, sort: &mut SortState) -> TreeAction {
    ...
    egui::ScrollArea::both().auto_shrink([false, false]).show(ui, |ui| {
        ...
        // 表格默认的 item_spacing 会在行之间留出几像素间距，这段间距不属于任何一个
        // 单元格，画的背景、感应的点击都盖不到，会变成"点了没反应"的死区，这里清零。
        ...
        builder
            .header(ROW_H, |mut h| {
                let cols = [(SortKey::Name, "名称"), (SortKey::LogicalSize, "大小"),
                    (SortKey::Modified, "修改时间"), (SortKey::Path, "路径")];
                for (key, label) in cols {
                    h.col(|ui| {
                        let text = if sort.key == Some(key) {
                            format!("{label} {}", if sort.ascending { "▲" } else { "▼" })
                        } else {
                            label.to_string()
                        };
                        let btn = egui::Button::new(egui::RichText::new(text).strong().size(12.0).color(Color32::WHITE)).frame(false);
                        if ui.add(btn).clicked() {
                            header_clicked.set(Some(key));
                        }
                    });
                }
            })
            .body(|body| {
                ...
                body.rows(ROW_H, flat_rows.len(), |mut row| {
                    ...
                    // 每一格开头都先画好背景（选中蓝底 / 隐藏淡橙底），四格连起来
                    // 视觉上就是一整行，不是只有名称那一格有背景。
                    let paint_bg = |ui: &mut egui::Ui| -> egui::Rect {
                    ...
                    // 整行的点击/右键判定：用 egui_extras 官方提供的 row.response()
                    // （目前为止加入这一行的所有单元格 Response 的并集），覆盖整行、
                    // 没有缝隙，而不是自己在每个格子里各判断一遍。
                    let resp = row.response();
                    if resp.clicked() || resp.secondary_clicked() {
                        clicked_row.set(Some(row_idx));
                    }
                    if !is_group {
...
/// `raw_arg` 会原样追加文本，不做任何转义，这样发给 explorer.exe 的命令行
/// 就和在 cmd.exe 里手动敲的完全一样。
#[cfg(windows)]

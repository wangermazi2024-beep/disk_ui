...
use crate::export;
use crate::model::{CategoryStat, Node, NodePath, SortState};
...
pub struct DiskUiApp {
    ...
    /// 点表头排序的状态。三个列表共用——点头一次排哪列，那一列的排序就一直生效，
    /// 直到再点别的表头。真正的排序发生在这个状态"变化"的那一刻（见 apply_sort_if_changed），
    /// 不是每帧都重排，所以不会有性能问题。
    sort: SortState,
}
...
impl DiskUiApp {
    fn show_main_screen(&mut self, ui: &mut egui::Ui) {
                ...
                ui.add_enabled_ui(background_enabled, |ui| {
                    match self.tabs.get(tab_idx) {
                        ...
                        Some(Tab::Extensions { root, selected, .. }) | Some(Tab::Duplicates { root, selected, .. }) => {
                            crate::ui::compact_tree::show(ui, root, selected, &mut self.sort)
                        }
                    }
                }).inner
            })
        ...
        if self.sort != sort_before {
            self.apply_sort_to_all_loaded_trees();
        }
    }

    /// 排序状态变了（用户点了表头）之后，把新的排序应用到*所有已经加载的树*——
    /// 主列表的每个分区 + 每个已打开的分析标签页，不只是当前看到的那一个。
    /// 这样切标签页的时候不会出现"这个标签页还是老排序"的不一致，代价是多排几棵树，
    /// 但这只在用户点表头这个离散动作发生时才会跑一次，不是每帧都排，开销可以忽略。
    fn apply_sort_to_all_loaded_trees(&mut self) {
        let Some(key) = self.sort.key else { return };
        for p in &mut self.partitions {
            p.sort_recursive(key, self.sort.ascending);
        }
        for t in &mut self.tabs {
            match t {
                Tab::Extensions { root, .. } | Tab::Duplicates { root, .. } => {
                    root.sort_recursive(key, self.sort.ascending);
                }
                Tab::Main => {}
            }
        }
    ...
    fn open_extension_tab(&mut self, pi: usize) {
        ...
        let mut root = categorize::build_extension_tree(node, &root_path);
    ...
    fn open_duplicate_tab(&mut self, pi: usize) {
        ...
        let Some(node) = self.partitions.get(pi) else { return };
        let root_path = self.partition_root_paths.get(pi).cloned().unwrap_or_default();
        let mut root = categorize::build_duplicate_tree(node, &root_path);
        if let Some(key) = self.sort.key {
            root.sort_recursive(key, self.sort.ascending);
        }
        let title = node.name.clone();
